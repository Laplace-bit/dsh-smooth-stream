/* probe conversation terminal state */
...
const st = await page.evaluate(() => ({
  phases: (typeof window.__report === 'function' ? (window.__phasesTrap || null) : null),
  foldedRows: document.querySelectorAll('.processedRow').length,
  reportReady: window.__reportReady,
}))
// fetch phases via a second run's trap: just report main body state now
console.log(JSON.stringify({ foldedRows: st.foldedRows, reportReady: st.reportReady }))
